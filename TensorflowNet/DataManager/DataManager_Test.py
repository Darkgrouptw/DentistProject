...
class DataManager:
    def __init__(
        ...
        # 讀檔案
        self._ReadOnlyForTesting(FileNameList)
    ...
    def TestFullImage(self, index):
        ...
        InputData = self._GetWindowImgFromPos(self.IndexArray[startIndex:endIndex])
    ...
    # 把整張圖讀進來
    def _ReadOnlyForTesting(self, FileNameList):
        print("Start Read Testing Data!")
        ...
        for i in range(len(FileNameList)):          # 跳過最後一個 \n
            ...
            for j in tqdm(range(len(FileNameList[i]))):
                ...
                # 確保有讀到資料
                assert type(InputImg) is np.ndarray
        ...
        # 讀完檔案
        print("Finish Read Testing Data!")
    ...
    def _GetWindowImgFromPos(self, choosePos):
        ...
        return WindowData.reshape([size, self.WindowsSize, self.WindowsSize, 1])